...
`custom-theme.css` remains the only incomplete roadmap item. The Phase 7 cascade experiment directly proved that removing its final late application changes 2–37% of homepage pixels, so deleting or mechanically renaming it would violate the locked behavior-preservation requirement. Its remaining rules require additional vertical-slice ownership checkpoints with fixture coverage; this checkpoint does not disguise the file under a new name.
